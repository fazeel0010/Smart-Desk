...
public class ScreenWorkerSignup extends AppCompatActivity {

    private Activity context;

    private PageStepIndicator indicator;
    private ViewPager viewPager;
    private PagerAdapterMechanicSignUP _pagerAdapterMechanicSignUP;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        context = this;
        Constants.const_MechanicSignupDTO = new SignupMechanicDTO();
        Constants.const_MechanicSignupDTO.setRole(Constants.workerRole);
        Constants.const_MechanicSignupDTO.setOnline(true);
        Constants.const_MechanicSignupDTO.setHired(false);
        actionbar();
        UtilityFunctions.setupUI(findViewById(R.id.parent), context);
        initLoadingBarItems();
        nextBtn = findViewById(R.id.nextbtn);
        linearHide = findViewById(R.id.linearHide);
        _pagerAdapterMechanicSignUP = new PagerAdapterMechanicSignUP(this, getSupportFragmentManager());

        viewPager = findViewById(R.id.view_pager);
        indicator = findViewById(R.id.tabs);
        viewPager.setAdapter(_pagerAdapterMechanicSignUP);
    ...
    public void changeStateOnValid() {
        Constants.const_MechanicSignupDTO.setWorkerLat(Constants.const_lat + 0.00030);
        Constants.const_MechanicSignupDTO.setWorkerLng(Constants.const_lng + 0.00030);
        int position = viewPager.getCurrentItem();
        FragmentWorkerStep1 fragmentWorkerStep1 = (FragmentWorkerStep1) getReferenceFragment(0);
        FragmentWorkerStep2 fragmentWorkerStep2 = (FragmentWorkerStep2) getReferenceFragment(1);
        if (position == 0) {
            fragmentWorkerStep1.validationsAndGetValues();
            if (isOkayStep1) {
                changePage(1);
            }
        } else if (position == 1) {
            fragmentWorkerStep2.validationsAndGetValues();
            fragmentWorkerStep1.validationsAndGetValues();
            if (!isOkayStep1)
                changePage(0);
            else if (!isOkayStep2)
                ...
                ((ScreenWorkerSignup) context).startAnim();
                try {
                    Query queryNumber = FirebaseConstants.firebaseFirestore.collection(FirebaseConstants.usersCollection).whereEqualTo("workerPhone", Constants.const_MechanicSignupDTO.getWorkerPhone());
                    queryNumber.get().addOnCompleteListener(task -> {
                        try {
                            if (task.isSuccessful()) {
                                if (!task.getResult().isEmpty()) {
                                    isOkayStep1 = false;
                                    fragmentWorkerStep1.met_phoneNumber.setError("Phone Number already registered");
                                    UtilityFunctions.orangeSnackBar((Activity) context, "Phone Number already registered!", Snackbar.LENGTH_SHORT);
                                }
                                Query queryCnic = FirebaseConstants.firebaseFirestore.collection(FirebaseConstants.usersCollection).whereEqualTo("workerEmail", Constants.const_MechanicSignupDTO.getWorkerEmail());
                                queryCnic.get().addOnCompleteListener(querySnapshotTask -> {
                                    try {
                                        if (querySnapshotTask.isSuccessful()) {
                                            ((ScreenWorkerSignup) context).stopAnim();
                                            if (!querySnapshotTask.getResult().isEmpty()) {
                                                isOkayStep1 = false;
                                                fragmentWorkerStep1.et_email.setError("Email already registered");
    ...
    public void changePage(int page) {
        try {
            if (PagerAdapterMechanicSignUP.pagesCount != 2) {
                PagerAdapterMechanicSignUP.pagesCount = 2;
