...
public class FragmentWorkerStep1 extends Fragment implements View.OnClickListener {
    ...
    public FragmentWorkerStep1() {
    }

    public FragmentWorkerStep1(Context mContext) {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        view = inflater.inflate(R.layout.fragment_mechanic_step1, container, false);
        initIDS();
        makeAttributedText();
        setSpinnerForGender();
    ...
    private void initKeyBoardListener() {
        try {
            ...
            final View decorView = ((ScreenWorkerSignup) signupContext).getWindow().getDecorView();
                    ...
                    try {
                        ...
                        if (lastVisibleDecorViewHeight != 0) {
                            if (lastVisibleDecorViewHeight > visibleDecorViewHeight + MIN_KEYBOARD_HEIGHT_PX) {
                                ((ScreenWorkerSignup) signupContext).nextBtn.setVisibility(View.GONE);
                                ((ScreenWorkerSignup) signupContext).linearHide.setVisibility(View.GONE);
                            } else if (lastVisibleDecorViewHeight + MIN_KEYBOARD_HEIGHT_PX < visibleDecorViewHeight) {
                                ((ScreenWorkerSignup) signupContext).nextBtn.setVisibility(View.VISIBLE);
                                ((ScreenWorkerSignup) signupContext).linearHide.setVisibility(View.VISIBLE);
                            }
                        }
...
//                startActivity(intent);
    ...
    public void setLocatioEditText() {
        new Handler().postDelayed(() -> et_shopLocation.setText(UtilityFunctions.getAddressLatLng((Activity) signupContext, Constants.const_lat, Constants.const_lng)), 1000);
    }

    public void validationsAndGetValues() {
        ScreenWorkerSignup.isOkayStep1 = true;
        isfirstRequestFocus = false;
        Constants.const_MechanicSignupDTO.setShopLocation(et_shopLocation.getText().toString());
        Constants.const_MechanicSignupDTO.setWorkerName(getDataFromEditext(et_workerName, "Invalid Name", 3));
        Constants.const_MechanicSignupDTO.setWorkerEmail(getDataFromEditext(et_email, "Invalid Email", 5));

        if(!UtilityFunctions.isValidEmail(Constants.const_MechanicSignupDTO.getWorkerEmail())){
            ScreenWorkerSignup.isOkayStep1 = false;
        ...
        if (!UtilityFunctions.isValidName(Constants.const_MechanicSignupDTO.getWorkerName())) {
            ScreenWorkerSignup.isOkayStep1 = false;
        ...
        Constants.const_MechanicSignupDTO.setWorkerPhone(getDataFromMaskText(met_phoneNumber, "Invalid Number", 12));

        try {
            Constants.const_MechanicSignupDTO.setWorkerPhone(Constants.const_MechanicSignupDTO.getWorkerPhone().replaceAll("-", ""));
        } catch (Exception ex) {
            ex.printStackTrace();
        }

        if (!Constants.const_MechanicSignupDTO.getWorkerPhone().startsWith("07")) {
            met_phoneNumber.setError("Number should start from 07");
            ScreenWorkerSignup.isOkayStep1 = false;
        }

        Constants.const_MechanicSignupDTO.setWorkerDob(et_dob.getText().toString());

        if (Constants.const_MechanicSignupDTO.getWorkerDob() == null) {
            ScreenWorkerSignup.isOkayStep1 = false;
            Toast.makeText(signupContext, "Please set Date of Birth", Toast.LENGTH_SHORT).show();
        }
        Constants.const_MechanicSignupDTO.setWorkerPassword(getDataFromEditext(et_password, "Invalid Password", 6));
        if (Constants.const_MechanicSignupDTO.getWorkerPassword().length() < 6) {
        ...
        if (ScreenWorkerSignup.isOkayStep1) {
            if (!Constants.const_MechanicSignupDTO.getWorkerPassword().equals(cPass)) {
                ScreenWorkerSignup.isOkayStep1 = false;
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.termscheckbox) {
            if (termscheckbox.isChecked()) {
                ((ScreenWorkerSignup) signupContext).nextBtn.setBackgroundTintList(ContextCompat.getColorStateList(signupContext, R.color.SmartDesk_Blue));
                ((ScreenWorkerSignup) signupContext).nextBtn.setEnabled(true);
            } else {
                ((ScreenWorkerSignup) signupContext).nextBtn.setBackgroundTintList(ContextCompat.getColorStateList(signupContext, R.color.SmartDesk_Blue_Oppaque));
                ((ScreenWorkerSignup) signupContext).nextBtn.setEnabled(false);
                ScreenWorkerSignup.isOkayStep1 = false;
            }
        }
    }
        ...
        genderSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                if (parent.getItemAtPosition(position).toString().equalsIgnoreCase(Constants.genderSelection)) {
                    ...
                    Constants.const_MechanicSignupDTO.setWorkerGender(parent.getItemAtPosition(position).toString());
