...
public class ScreenConsumerSignup extends AppCompatActivity {

    private Activity context;

    private PageStepIndicator indicator;
    private ViewPager viewPager;
    private PagerAdapterConsumerSignUP pagerAdapterConsumerSignUP;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        context = this;
        Constants.const_ConsumerSignupDTO = new SignupConsumerDTO();
        Constants.const_ConsumerSignupDTO.setRole(Constants.consumerRole);
        Constants.const_ConsumerSignupDTO.setHired(false);
        Constants.const_ConsumerSignupDTO.setWorkerDocumentID("");
        ...
        pagerAdapterConsumerSignUP = new PagerAdapterConsumerSignUP(this, getSupportFragmentManager());

        viewPager = findViewById(R.id.view_pager);
        indicator = findViewById(R.id.tabs);
        viewPager.setAdapter(pagerAdapterConsumerSignUP);
...
//                                Toast.makeText(this, "Permission Deny", Toast.LENGTH_SHORT).show();
    ...
    private void actionbar() {
        ...
        ((TextView) findViewById(R.id.actionbarInclude).findViewById(R.id.actionTitleBar)).setText("Consumer Registration");
    ...
    public void changeStateOnValid() {
        Constants.const_ConsumerSignupDTO.setWorkerLat(Constants.const_lat);
        Constants.const_ConsumerSignupDTO.setWorkerLng(Constants.const_lng);
        int position = viewPager.getCurrentItem();
        FragmentConsumerStep1 fragmentConsumerStep1 = (FragmentConsumerStep1) getReferenceFragment(0);
        fragmentConsumerStep1.validationsAndGetValues();
        if (position == 0) {
            if (isOkayStep1)
                changePage(1);
        } else if (position == 1) {
            if (!isOkayStep1)
                ...
                try {
                    startAnim();
                    Query queryNumber = FirebaseConstants.firebaseFirestore.collection(FirebaseConstants.usersCollection).whereEqualTo("workerPhone", Constants.const_ConsumerSignupDTO.getWorkerPhone());
                    queryNumber.get().addOnCompleteListener(task -> {
                        try {
                            ((ScreenConsumerSignup) context).stopAnim();
                            if (task.getResult().isEmpty()) {
                                new Handler().postDelayed(() -> showAlertDialog(context, "Are you sure, you want to create account with the provided information?"), 0);
                            } else {
                                isOkayStep1 = false;
                                ((ScreenConsumerSignup) context).changePage(0);
                                fragmentConsumerStep1.met_phoneNumber.setError("Phone Number already registered");
    ...
    private void changePage() {
        ...
        try {
            if (PagerAdapterConsumerSignUP.pagesCount != 2) {
                PagerAdapterConsumerSignUP.pagesCount = 2;
